...
2. Set these environment variables in the Render dashboard:
	DATABASE_URL, FERNET_SECRET_KEY, ADMIN_SECRET_TOKEN, TELEGRAM_BOT_TOKEN, OPENAI_API_KEY, SQL_GENERATION_MODEL, RESPONSE_FORMAT_MODEL, OFF_TOPIC_CLASSIFIER_MODEL, SQL_DEFAULT_ROW_LIMIT, SQL_FULL_ROW_LIMIT
...
- Off-topic classification uses OpenAI Chat Completions (`OFF_TOPIC_CLASSIFIER_MODEL`, default `DB_ROUTER_MODEL` / `gpt-4o-mini`).
- Tenant-specific schema blueprint is stored in `tenant_db_credentials.schema_blueprint`.
- For PostgreSQL and Google Sheets tenants, `schema_blueprint` stores only `metadata_analysis.json`-style semantic metadata. Runtime table/column details and live sheet rows are fetched at message time and are not saved into `schema_blueprint`.
